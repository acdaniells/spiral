...
docs = {
    ...
    "line_dash_map": [
        ...
        " dash-patterns to lines corresponding with specific values. Keys in"
        " `line_dash_map` should be values in the column denoted by"
        " `line_dash`.",
    ],
    "line_dash_sequence": [
        "list of str",
    ...
    "color_discrete_sequence": [
        ...
        " `color_discrete_sequence` in the order described in"
        " `category_orders`, unless the value of `color` is a key in"
        " `color_discrete_map`. Various useful color sequences are available in"
        " the `plotly.express.colors` submodules, specifically"
    ...
    "color_discrete_map": [
        ...
        " corresponding with specific values. Keys in `color_discrete_map`"
    ...
    "category_orders": [
        ...
        " Python below 3.6). This parameter is used to force a specific"
        " ordering of values per column. The keys of this dict should"
        " correspond to column names, and the values should be lists of strings"
        " corresponding to the specific display order desired.",
    ],
    "marginal": [
        "str",
        "One of `'rug'`, `'box'`, `'violin'`, or `'histogram'`. If set, a"
        " subplot is drawn alongside the main plot, visualizing the"
        " distribution.",
    ],
    "marginal_x": [
        "str",
    ...
    "trendline": [
        ...
        " regression line will be drawn for each discrete-color/symbol group."
        " If `'lowess`', a Locally Weighted Scatterplot Smoothing line will be"
        " drawn for each discrete-color/symbol group.",
    ],
    "trendline_color_override": [
        "str",
        "Valid CSS color. If provided, and if `trendline` is set, all"
        " trendlines will be drawn in this color.",
    ],
    "render_mode": [
        "str",
        "One of `'auto'`, `'svg'` or `'webgl'`, default `'auto'`. Controls the"
        " browser API used to draw marks. `'svg`' is appropriate for figures of"
        " less than 1000 data points, and  will allow for fully-vectorized"
        " output. `'webgl'` is likely necessary for acceptable performance"
        " above 1000 points but rasterizes part of the output. `'auto'` uses"
        " heuristics to choose the mode.",
    ],
    "direction": [
        "str",
        "One of '`counterclockwise'` or `'clockwise'`. Default is"
        " `'clockwise'`. Sets the direction in which increasing values of the"
        " angular axis are drawn.",
    ],
    "start_angle": [
        "int (default `90`)",
    ...
    "histfunc": [
        ...
        " values of `y` if `orientation` is `'v'`, otherwise the arguments are"
        " the values of `x`. The arguments to this function for"
        " `density_heatmap` and `density_contour` are the values of `z`.",
    ],
    "histnorm": [
        "str (default `None`)",
        "One of `'percent'`, `'probability'`, `'density'`, or"
        " `'probability density'`. If `None`, the output of `histfunc` is used"
        " as is. If `'probability'`, the output of `histfunc` for a given bin"
        " is divided by the sum of the output of `histfunc` for all bins. If"
        " `'percent'`, the output of `histfunc` for a given bin is divided by"
        " the sum of the output of `histfunc` for all bins and multiplied by"
        " 100. If `'density'`, the output of `histfunc` for a given bin is"
        " divided by the size of the bin. If `'probability density'`, the"
        " output of `histfunc` for a given bin is normalized such that it"
        " corresponds to the probability that a random event whose distribution"
        " is described by the output of `histfunc` will fall into that bin.",
    ],
    "barnorm": [
        "str (default `None`)",
    ...
    "groupnorm": [
        "str (default `None`)",
        "One of `'fraction'` or `'percent'`. If `'fraction'`, the value of each"
        " point is divided by the sum of all values at that location"
        " coordinate. `'percent'` is the same but multiplied by 100 to show"
        " percentages. `None` will stack up all values at each location"
        " coordinate.",
    ],
    "barmode": [
    ...
    "boxmode": [
        "str (default `'group'`)",
        "One of `'group'` or `'overlay'`. In `'overlay'` mode, boxes are on"
        " drawn top of one another. In `'group'` mode, baxes are placed beside"
        " each other.",
    ],
    "violinmode": [
        "str (default `'group'`)",
        "One of `'group'` or `'overlay'`. In `'overlay'` mode, violins are on"
        " drawn top of one another. In `'group'` mode, violins are placed"
        " beside each other.",
    ],
    "stripmode": [
        "str (default `'group'`)",
    ...
    "scope": [
        "str (default `'world'`).",
        "One of `'world'`, `'usa'`, `'europe'`, `'asia'`, `'africa'`,"
        " `'north america'`, or `'south america'`). Default is `'world'` unless"
        " `projection` is set to `'albers usa'`, which forces `'usa'`.",
    ],
    "projection": [
        "str ",
        "One of `'equirectangular'`, `'mercator'`, `'orthographic'`,"
        ...
        " `'aitoff'`, or `'sinusoidal'`. Default depends on `scope`.",
    ],
    "center": [
        "dict",
    ...
    "mapbox_style": [
        ...
        " values which do not require a Mapbox API token are"
        " `'open-street-map'`, `'white-bg'`, `'carto-positron'`,"
        " `'carto-darkmatter'`, `'stamen-terrain'`, `'stamen-toner'`,"
        " `'stamen-watercolor'`. Allowed values which do require a Mapbox API"
        " token are `'basic'`, `'streets'`, `'outdoors'`, `'light'`, `'dark'`,"
    ...
    "points": [
        ...
        " shown. If `'suspectedoutliers'`, all outlier points are shown and"
        " those less than 4*Q1-3*Q3 or greater than 4*Q3-3*Q1 are highlighted"
        " with the marker's `'outliercolor'`. If `'outliers'`, only the sample"
        " points lying outside the whiskers are shown. If `'all'`, all sample"
        " points are shown. If `False`, no sample points are shown and the"
        " whiskers extend to the full range of the sample.",
    ],
    "box": [
        "boolean (default `False`)",
    ...
    "featureidkey": [
        ...
        " passed in to `locations`. The most common alternative to the default"
        " is of the form `'properties.<key>`.",
    ],
    "cumulative": [
        "boolean (default `False`)",
    ...
    "branchvalues": [
        ...
        " summed. When set to 'total', items in `values` are taken to be value"
        " of all its descendants. When set to 'remainder', items in `values`"
        " corresponding to the root and the branches:sectors are taken to be"
        " the extra part not part of the sum of the values at their leaves.",
    ],
    "maxdepth": [
        "int",
    ...
    "hole": [
        "float",
        "Sets the fraction of the radius to cut out of the pie. Use this to"
        " make a doughnut chart.",
    ],
}


def make_docstring(method, override_dict=None):
    """
    Make a docstring for a given method.
    """
    tw = TextWrapper(width=72, initial_indent="    ", subsequent_indent="    ")
    result = (method.__doc__ or "") + "\nParameters\n----------\n"

    for param in getfullargspec(method)[0]:
        if param == "self":
            continue

        if override_dict is not None and override_dict.get(param):
