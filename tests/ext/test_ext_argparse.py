...
from spiral.ext.ext_argparse import (
...
class Base(ArgparseController):
    class Meta:
        label = "base"
        arguments = [(["--foo"], dict(dest="foo", default="bar"))]
...
class Second(ArgparseController):
    class Meta:
        ...
        arguments = [(["--foo2"], dict(dest="foo2", default=1))]
...
class Third(ArgparseController):
    class Meta:
        ...
        stacked_on = "base"
        stacked_type = "nested"
        arguments = [(["--foo3"], dict(dest="foo3", default=["foo", "bar"]))]
...
def test_bad_alternative_default_command():
    ...
    with MyApp() as app:
        ...
        with raises(SpiralError, match=msg):
            app.run()
...
def test_invalid_stacked_type():
    ...
    with MyApp() as app:
        with raises(SpiralError, match="(.*)Invalid stacked type(.*)"):
            app.run()
