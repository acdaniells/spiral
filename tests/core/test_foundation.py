...
from spiral import App, Controller, ex
from spiral.core.foundation import (
    TestApp,
...
def test_loaded_extensions():
    ...
    with MyApp() as app:
        for ext in ext_list:
            loaded = app.ext._loaded_extensions
            cement_ext = f"cement.ext.ext_{ext}"
            spiral_ext = f"spiral.ext.ext_{ext}"
            assert cement_ext in loaded or spiral_ext in loaded
...
def test_passed_handlers():
    ...
    from spiral.ext.ext_argparse import ArgparseArgumentHandler
    ...
    with ThisApp("test-app", config_files=None) as app:
        ...
        if platform.system().lower() in ["windows"]:
            files = [
                ...
                os.path.join(user_home, f".{app.label}", "config", f"{app.label}.conf"),
            ]
        else:
            files = [
